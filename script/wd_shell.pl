...
if ( my $file = $opt{f} ) {
    pod2usage( -exitstatus => 2, -message => "Not exists file [-f] : $file" ) unless -e $file;
} else { pod2usage( -exitstatus => 2, -message => 'Need  file [-f] !')  unless $ENV{wdIndexFile} && -e $ENV{wdIndexFile} }
