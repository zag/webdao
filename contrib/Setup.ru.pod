...
wdIndexFile |" index.html - наименование файла, который будет обрабатываться при поступлении запросов. В качестве значения возможны: полный путь, относительно C<DOCUMENT_ROOT>. По умолчанию: F<$ENV{DOCUMENT_ROOT}/index.xhtml> "|  index.html
...
=item * Простой пример
	...
	error_log  /var/log/nginx/example.org-error.log  debug;
        ...
        location ~ / {
            ...
            fastcgi_param wdSession HTML::WebDAO::Sessionco;
...
Использование I<wd_shell.pl> устарело. Используйте L<WebDAO::Test>.
